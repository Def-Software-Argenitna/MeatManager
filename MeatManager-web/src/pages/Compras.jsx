    ...
    const handleAddPurchase = async (e) => {
        ...
        try {
            ...
            if (editingPurchaseId) {
                await saveTableRecord('compras', 'update', {
                ...
                const oldItems = (await fetchTable('compras_items', { limit: 500 })).filter(i => Number(i.purchase_id) === purchaseId);
                await Promise.all(oldItems.map(i => saveTableRecord('compras_items', 'delete', null, i.id)));
            } else {
                ...
                purchaseId = Number(purchaseInsert?.insertId);
            }
                        ...
                        <form onSubmit={handleAddPurchase}>
                            ...
                            <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem', borderTop: '1px solid var(--color-border)', paddingTop: '1rem' }}>
                                ...
                                <div style={{ flex: 1 }}></div>
                                    ...
                                    <button type="submit" className="neo-button" disabled={isSubmitting} style={{ opacity: isSubmitting ? 0.6 : 1 }}>
                                        <Save size={18} style={{ marginRight: '0.5rem' }} /> {isSubmitting ? 'Guardando...' : 'Guardar Compra'}
                                    </button>
                                </div>
                            </div>
                        </form>
