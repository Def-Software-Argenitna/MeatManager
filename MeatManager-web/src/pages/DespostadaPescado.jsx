...
const DespostadaPescado = () => {
    ...
    const confirmCut = async () => {
        ...
        const cutInfo = FISH_MAP.find(c => c.id === selectedCutId);
    ...
    const selectedCut = FISH_MAP.find(c => c.id === selectedCutId);
            ...
            <DirectionalReveal className="session-setup" from="up" delay={0.04}>
                <div style={{ display: 'flex', gap: '2rem', alignItems: 'center', flex: 1 }}>
                    <div className="weight-input-group">
                        <label>Stock Disponible (Piezas Enteras)</label>
                        <select
                            className="neo-input"
                        ...
                        {availableLots?.length === 0 && !isSessionStarted && (
                            <div style={{ fontSize: '0.75rem', color: 'orange', marginTop: '0.25rem' }}>
                                💡 Recordá registrar la COMPRA del pescado para que aparezca acá.
                            </div>
                        )}
                    </div>
            ...
            <div className="workspace-area" style={{ gridTemplateColumns: '280px 1fr 350px' }}>
                ...
                <DirectionalReveal className="cuts-list-panel" from="left" delay={0.1} style={{
                    ...
                    <div style={{ padding: '1rem', borderBottom: '1px solid var(--color-border)', fontWeight: '700', fontSize: '0.9rem', color: 'var(--color-text-muted)', textTransform: 'uppercase' }}>
                        Lista de Cortes ({FISH_MAP.length})
                    </div>
                ...
                <DirectionalReveal className="visual-map-container" from="up" delay={0.16} style={{ backgroundColor: '#111', padding: 0 }}>
                    <img
                        src="/pescado_argentino.png"
                        alt="Mapa de Cortes de Pescado"
                        style={{
                            width: '90%',
                            height: 'auto',
                            maxHeight: '90%',
