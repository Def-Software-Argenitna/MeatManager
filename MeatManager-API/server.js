...
app.get('/api/firebase-users', verifyFirebaseToken, async (req, res) => {
    try {
        ...
        try {
            ...
            licensePool = await getClientLicensePool(conn, accessContext.client.id);

            const users = [];
            for (const row of rows) {
                const perms = await getUserPermissions(conn, row.id);
                const assignedLicenses = licensePool.filter((license) => String(license.userId || '') === String(row.id));
                const baseUser = buildAccessResponse({
                    user: {
                        ...row,
                        userStatus: row.status,
                        branchRecordId: row.branchId,
                        perms,
                    },
                    client: accessContext.client,
                    effectiveLicenses: assignedLicenses.map((license) => ({
                        clientLicenseId: license.id,
                        licenseId: license.licenseId,
                        commercialName: license.license.commercialName,
                        internalCode: license.license.internalCode,
                        category: license.license.category,
                        billingScope: license.license.billingScope,
                        assignedUserId: license.userId,
                        assignedBranchId: license.branchId,
                        appliesToWebapp: license.license.appliesToWebapp,
                        featureFlags: license.license.featureFlags,
                    })),
                    deliveryLicenses: assignedLicenses
                        .filter((license) => license.license.hasLogisticsCapability)
                        .map((license) => ({
                            clientLicenseId: license.id,
                            licenseId: license.licenseId,
                            commercialName: license.license.commercialName,
                ...
                users.push({
                    ...baseUser,
                    perms,
                    assignedLicenses: assignedLicenses.map((license) => ({
                        clientLicenseId: license.id,
                        licenseId: license.licenseId,
                        commercialName: license.license.commercialName,
                        internalCode: license.license.internalCode,
                        billingScope: license.license.billingScope,
                        hasLogisticsCapability: license.license.hasLogisticsCapability,
                    })),
                });
            }
